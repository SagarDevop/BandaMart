...
export default function ProductManager() {
  ...
  const openAdd = () => {
    ...
    setForm({ name: '', category: categories[0]?.id || '', price: '', unit: 'kg', description: '', available: true, featured: false, image: '' });
    setShowModal(true);
  };
            ...
            <div key={product.id} style={{
              ...
              <div style={{ flexGrow: 1, minWidth: 0 }}>
                ...
                <p className="text-label-sm" style={{ color: 'var(--outline)', margin: '2px 0 0' }}>
                  ₹{product.price}/{product.unit} · {categories.find(c => c.id === product.category)?.name}
                </p>
              </div>
          ...
          <div onClick={e => e.stopPropagation()} style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-md)' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-md)' }}>
                <div>
                  <label className="text-label-sm" style={{ display: 'block', color: 'var(--on-surface-variant)', marginBottom: 4 }}>Price (₹) *</label>
